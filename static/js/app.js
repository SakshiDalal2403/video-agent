...
function setStatusChip(mode, label) {
    ...
    // Preserve the status-dot span and update only the label text
    const dot = elements.statusChip.querySelector(".status-dot");
    elements.statusChip.innerHTML = "";
    if (dot) {
        elements.statusChip.appendChild(dot);
    } else {
        const newDot = document.createElement("span");
        newDot.className = "status-dot";
        elements.statusChip.appendChild(newDot);
    }
    elements.statusChip.appendChild(document.createTextNode(" " + label));
}
...
function updateResultVisibility(hasResult) {
    ...
    // heroPanel is now the permanent left sidebar — never hide it
}
...
async function fetchState() {
    ...
    if (response.status === 401) {
        // Show login modal if session is expired
        showLoginModal();
...
window.addEventListener("load", async () => {
    ...
    // Check if token exists in localStorage, if not show Login modal
    const token = localStorage.getItem("jwt_token");
    if (!token) {
        showLoginModal();
        return;
